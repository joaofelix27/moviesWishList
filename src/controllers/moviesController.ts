...
import { CreateMoviesData, moviesService, UpdateMoviesData } from "../services/moviesService.js";
...
async function updateMovie(req: Request, res: Response) {
  const movie: UpdateMoviesData = req.body;
  const movies = await moviesService.updateMovie(movie);
  res.sendStatus(201)
}
